...
async function fbSignup({ displayName, userId, email, password, joinedDate }) {
  try {
    ...
    if (users[userId]) return { ok: false, error: "Username already taken" };
    const newUser = {
      ...
      safety_accepted: null, invite_code: "", invites_count: 0, burned_at: null,
    ...
    localStorage.setItem("kd_current_uid", userId);
...
async function fbLogout() {
  try { localStorage.removeItem("kd_token"); localStorage.removeItem("kd_current_uid"); } catch {}
}
